...
from engine_differentiable import train_one_epoch, evaluate
...
def get_args_parser():
    parser = argparse.ArgumentParser('Differentiable AdaptiveNN training script', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='dynamic_deitS_diff', type=str, metavar='MODEL',
    ...
    parser.add_argument('--project', default='AdaptiveNN_Differentiable', type=str,
...
def main(args):
    ...
    if args.finetune:
        ...
        for k, v in checkpoint_model.items():
            # 跳过PPO特有的组件
            if 'policy_net_patch_stateV' in k:
                print(f"Skipping PPO-specific key: {k}")
                continue
            # 处理输出头维度不匹配
            if 'policy_net_patch.output_head' in k:
                if k in state_dict and v.shape != state_dict[k].shape:
                    print(f"Skipping key {k} due to shape mismatch: {v.shape} vs {state_dict[k].shape}")
                    continue
            # 处理分类头
            if k in ['head.weight', 'head.bias'] and k in state_dict:
                if v.shape != state_dict[k].shape:
                    print(f"Removing key {k} from pretrained checkpoint")
                    continue
            new_checkpoint[k] = v
        
        utils.load_state_dict(model, new_checkpoint, prefix=args.model_prefix)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Differentiable AdaptiveNN training script', parents=[get_args_parser()])
