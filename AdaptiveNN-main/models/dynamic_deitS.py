                ...
                if self.training:
                    with torch.cuda.amp.autocast(enabled=False):
                        ...
                        if torch.isnan(actions).any():
                            print(f"Warning: NaN detected in actions at step {focus_step_index}, replacing with 0.5")
                            actions = torch.nan_to_num(actions, nan=0.5)
                        # Clamp actions to valid range before creating distribution
                        actions = torch.clamp(actions, min=0.0, max=1.0)
