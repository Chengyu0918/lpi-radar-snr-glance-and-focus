...
class NativeScalerWithGradNormCount:
    ...
    def __call__(
        self, loss, optimizer, model=None, clip_grad=None, parameters=None, create_graph=False, update_grad=True,
        skip_backbones=False, skip_policy_net=False
    ):
    ...
    if args.auto_resume and len(args.resume) == 0:
        ...
        if latest_ckpt >= 0:
            ...
            try:
                ...
                args.resume = os.path.join(output_dir, 'checkpoint-%d.pth' % (latest_ckpt - 1))
                print("Save unsuccessfully! Auto resume checkpoint: %s" % args.resume)
